...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    for i in tweet_data['statuses']:
        for j in i['entities']['hashtags']:
            list_of_hashtags.append(j['text'].lower())
    hashtag_counter = {}
    for i in list_of_hashtags:
        if i == hashtag_to_ignore.lower():
            continue
        else:
            hashtag_counter[i] = hashtag_counter.get(i, 0) + 1
    most_repeated = max(hashtag_counter, key=hashtag_counter.get)
...
if __name__ == "__main__":
    ...
    hashtag = "MarchMadness2021" ### I removed the # because in tweet data '#' is not included in the string
    count = 100
